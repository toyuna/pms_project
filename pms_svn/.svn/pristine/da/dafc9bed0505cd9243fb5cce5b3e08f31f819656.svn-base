...
public class MeetingService {
	...
	public String insertMeeting(Meeting ins) {
			...
			try {
				for (MultipartFile mf : ins.getReport()) {
					...
					if (fname != null && !fname.equals("")) {
						...
						mf.transferTo(file);
						// 첨부파일 정보 DB에 등록..
						dao.insertFileInfo(new FileInfo(uploadPath, fname, "게시물 제목:" + ins.getMeeting_title()));
					}
				}
	...
	public Meeting getMeeting(int meeting_list_num) {
		// 상세화면 호출 시 조회수 증가
		dao.updateReadCnt(meeting_list_num);
		// notice안에 상세정보와 다운로드할 파일정보를 설정되게함.
		Meeting m = dao.getMeeting(meeting_list_num);
		m.setFnames(dao.getFileInfo(meeting_list_num));
		return m;
	}
