...
public class NoticeService {
	...
	public String insertNotice(Notice ins) {
		...
		String msg = "등록성공";
		// 특정한 위치에 첨부파일 업로드.
		System.out.println("#####첨부파일 있는지?:"+ins.getReport()!=null);
			...
			try {
				for (MultipartFile mf : ins.getReport()) {
					...
					if (fname != null && !fname.equals("")) {
						...
						mf.transferTo(file);
						// 첨부파일 정보 DB에 등록..
						dao.insertFileInfo(new FileInfo(uploadPath, fname, "게시물 제목:" + ins.getNotice_title()));
					}
				}
	...
	public Notice getNotice(int notice_list_num) {
		// 상세화면 호출 시 조회수 증가
		dao.updateReadCnt(notice_list_num);
		// notice안에 상세정보와 다운로드할 파일정보를 설정되게함.
		Notice n = dao.getNotice(notice_list_num);
		n.setFnames(dao.getFileInfo(notice_list_num));
		System.out.println(notice_list_num+" #번호");
		System.out.println(n.getFnames().size()+" #파일 갯수");
		return n;
	}
