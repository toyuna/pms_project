...
public class ChattingHandler extends TextWebSocketHandler{
	...
	// 2. 소켓 서버에 메세지 전송 처리 메서드
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		// 1) 특정한 client를 통해 전달해 온 메세지를 출력
			log(session.getId()+"에서 온 메세지:"+message.getPayload());
		// 2) 현재 접속한 모든 계정에 메세지 전달
			for(WebSocketSession ws:users.values()) {
				// 1. 각 접속한 클라이언트에게 메세지 전달 
				ws.sendMessage(message);
				// 2. 각 전달할 사용자 및 메세지 확인
				log(ws.getId()+"에게 전달 메세지:"+message.getPayload());
			}
	}
