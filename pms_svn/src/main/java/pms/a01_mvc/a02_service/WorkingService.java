...
import pms.a01_mvc.a03_dao.WorkingDao;
...
public class WorkingService {
	...
	public String insertWorking(Working ins) {
		dao.insertWorking(ins);
		String msg = "등록성공";
		// 특정한 위치에 첨부파일 업로드.
			...
			try {
				for (MultipartFile mf : ins.getReport()) {
					...
					if (fname != null && !fname.equals("")) {
						...
						mf.transferTo(file);
						// 첨부파일 정보 DB에 등록..
						dao.insertFileInfo(new FileInfo(uploadPath, fname, "게시물 제목:" + ins.getWorking_title()));
					}
				}
	...
	public Working getWorking(int working_list_num) {
		// 상세화면 호출 시 조회수 증가
		dao.updateReadCnt(working_list_num);
		// notice안에 상세정보와 다운로드할 파일정보를 설정되게함.
		Working w = dao.getWorking(working_list_num);
		w.setFnames(dao.getFileInfo(working_list_num));
		return w;
	}
	
	
	public void delWorking(int working_list_num) {
		dao.delWorking(working_list_num);
	}
	
	
	
	public void uptWorking(Working upt){
		dao.uptWorking(upt);
	}
